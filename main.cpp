...
#if TARGET_NAME == TINY2040
DigitalOut led(p18);
DigitalOut LED_B(p20, 1);
DigitalOut LED_G(p19, 1);
#else
DigitalOut led(LED1);
#endif
...
int main()
{
    ...
    while (1) {
        ...
        // サイバネ領域
        if (rcs620s.polling(CYBERNE_SYSTEM_CODE) || rcs620s.polling(SAPICA_SYSTEM_CODE) || rcs620s.polling(PASPY_SYSTEM_CODE)) {
